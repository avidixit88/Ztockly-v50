...
killzone_preset = st.sidebar.selectbox(
    "Killzone preset",
    ["Custom (use toggles below)", "Opening Drive", "Lunch Chop", "Power Hour", "Pre-market"],
    index=0,
    help="Quick presets for time windows + liquidity weighting."
)
liquidity_weighting = st.sidebar.slider("Liquidity-weighted scoring (0–1)", 0.0, 1.0, 0.55, 0.05,
    help="Weights signals more heavily during high-liquidity periods (open/close) and less during chop.")
orb_minutes = st.sidebar.slider("ORB window (minutes)", 5, 60, 15, 5, help="Opening range window for ORB levels.")
...
# Build time-of-day filter list from toggles
time_of_day_filter = []
if opening: time_of_day_filter.append('OPENING')
if midday: time_of_day_filter.append('MIDDAY')
if power: time_of_day_filter.append('POWER')
if premarket: time_of_day_filter.append('PREMARKET')
if afterhours: time_of_day_filter.append('AFTERHOURS')
...
st.divider()
with st.expander("Replay mode (historical bar-by-bar)", expanded=False):
    st.caption("Runs historical bars sequentially to validate confirmation/cooldown/email without waiting live.")
    rep_symbol = st.text_input("Replay ticker", value="AAPL")
    rep_bars = st.slider("Number of bars to replay", 50, 500, 200, 25)
    rep_sleep_ms = st.slider("Step delay (ms)", 0, 500, 0, 25, help="For visual stepping; keep 0 for fastest.")
    run_replay_btn = st.button("Run replay", key="run_replay_btn", use_container_width=True)

    if run_replay_btn:
        try:
            ohlcv_full, rsi5_full, rsi14_full, macd_hist_full, quote_full = fetch_bundle(client, rep_symbol, interval=interval)
            bundle = {"ohlcv": ohlcv_full, "rsi5": rsi5_full, "rsi14": rsi14_full, "macd_hist": macd_hist_full}
        except Exception as e:
            st.error(f"Replay fetch failed: {e}")
            st.stop()
        ohlcv_full = bundle["ohlcv"].reset_index(drop=True)
        ohlcv = ohlcv_full.tail(rep_bars).reset_index(drop=True)

        replay_alerts = []
        for i in range(20, len(ohlcv)):
            o = ohlcv.iloc[: i + 1].copy()
            ...
            def _slice(name):
                ...
                if s is None:
                    return None
                try:
                    return s.tail(rep_bars).reset_index(drop=True).iloc[: i + 1]
                except Exception:
                    return None
            ...
            sig = compute_scalp_signal(
                ...
                target_atr_pct=target_atr_pct,
                use_last_closed_only=False,      # replay already steps bar-by-bar
                bar_closed_guard=False,
                interval_mins=interval_mins,
                entry_model=entry_model,
                slip_mode=slip_mode,
                slip_fixed_cents=slip_fixed_cents,
                slip_atr_frac=slip_atr_frac,
                bias_strictness=bias_strictness,
                split_long_short=split_long_short,
            ...
            if sig is not None:
                row = sig.to_dict()
                if row.get("Bias") in ("LONG","SHORT") and row.get("Entry") is not None:
                    replay_alerts.append(row)

            if rep_sleep_ms:
                import time as _t
                _t.sleep(rep_sleep_ms/1000.0)

        st.write("Replay alerts:", len(replay_alerts))
        if replay_alerts:
            st.dataframe(replay_alerts[-50:], use_container_width=True)
